...
from lmfit import minimize, Parameters, Parameter,\
                    report_fit, Model
from lmfit.models import StepModel,PolynomialModel, ExponentialModel, PowerLawModel, DonaichModel, GaussianModel, LorentzianModel, ConstantModel, RectangleModel, LinearModel


def gaussian(x, height, center, width, offset):
    return (height/np.sqrt(2*np.pi)*width) * np.exp(-(x - center)**2/(2*width**2)) + offset

def polynomial(c0,c1,c2,i,sl,y):
    return c0 + c1*y + c2*(y**2) + sl*y + i# + c3*(y**3) 

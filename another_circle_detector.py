...
def select_area_for_detector(np_image):
    ...
    # edges_bin = dilation(edges_bin, selem=square(3))
    edges_bin = edges >= threshold
    pl.subplot(1,3,2)
    pl.imshow(edges_bin)
    
    label_image = label(edges_bin, neighbors=4)
    pl.subplot(1,3,3)
    pl.imshow(label_image)
    pl.show()
...
def detect_circles(np_image):
    ...
    return areas, circles, C, bord, radii
...
"""
The extracted circle info is the used to generate an image
of a single circle in a box, which is used for testing the radii function.
"""
empt = np.zeros(area[0].shape)
empt[circ[0][0], circ[0][1]] = 255
misc.imsave("segmented_real.png", empt)
print C[0]
print R[0]
pl.imshow(empt, cmap=pl.cm.Greys)
pl.show()

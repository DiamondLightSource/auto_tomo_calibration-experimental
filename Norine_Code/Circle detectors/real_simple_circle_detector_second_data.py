...
def detect_circles(np_image):
    ...
    areas, bord = select_area_for_detector(np_image)
    ...
    for i in range(0, len(areas)):
        ...
        or areas[i].shape[0] <= 120 or areas[i].shape[1] <= 120:
            index.append(i)
            continue
    
    if index != []:
        areas = np.delete(areas, index)
        bord = np.asarray(bord)
        bord = np.delete(bord, index, 0)
    print 'Length of areas after selection: ', len(areas)
    print 'Borders:'
    print bord
    ...
    for i in range(1, len(areas)):
        ...
        if (abs(bord[i][0] - bord[i-1][0]) <= 30 and abs(bord[i][2] - bord[i-1][2]) <= 30):
            index.append(i)
            continue
    ...
    print 'Borders:'
    print bord
    #draw_areas(np_image, areas, bord)
    ...
    for i in range(len(areas)):
        np_image_norm[circles[i][0] + bord[i][0], circles[i][1] + bord[i][2]] = np.min(np_image_norm)
        # To thicken the line
        np_image_norm[circles[i][0]-1 + bord[i][0], circles[i][1]-1 + bord[i][2]] = np.min(np_image_norm)
        np_image_norm[circles[i][0]-1 + bord[i][0], circles[i][1] + bord[i][2]] = np.min(np_image_norm)
        np_image_norm[circles[i][0]-1 + bord[i][0], circles[i][1]+1 + bord[i][2]] = np.min(np_image_norm)
        np_image_norm[circles[i][0] + bord[i][0], circles[i][1]-1 + bord[i][2]] = np.min(np_image_norm)
        np_image_norm[circles[i][0] + bord[i][0], circles[i][1]+1 + bord[i][2]] = np.min(np_image_norm)
        np_image_norm[circles[i][0]+1 + bord[i][0], circles[i][1]-1 + bord[i][2]] = np.min(np_image_norm)
        np_image_norm[circles[i][0]+1 + bord[i][0], circles[i][1] + bord[i][2]] = np.min(np_image_norm)
        np_image_norm[circles[i][0]+1 + bord[i][0], circles[i][1]+1 + bord[i][2]] = np.min(np_image_norm)
    
    dnp.plot.image(np_image_norm)
    pl.imshow(np_image_norm, cmap=pl.cm.YlOrRd)

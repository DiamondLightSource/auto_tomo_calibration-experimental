...
def select_area_sphere(np_image): # to select the cubes containing the spheres
    ...
    for slice in range(N):
        ...
        #    print 'Len(areas):', len(areas_circle)
        ...
        for i in range(0, len(areas_circle)):
            ...
            if areas_circle[i].shape[0] >= 1000 or areas_circle[i].shape[1] >= 1000:
                #print 'in if 1'
                index.append(i)
                continue
        ...
        for i in range(1, len(areas_circle)):
            ...
            '''TO BE CHANGED TO FIT DATA'''
            if (abs(bord_circle[i][0] - bord_circle[i-1][0]) <= 100 and abs(bord_circle[i][2] - bord_circle[i-1][2]) <= 100):
                #print 'in if 2'
                index.append(i)
                continue
    ...
    '''ATTENTION: CENTROIDS ARE CENTRES IN EVERY AREA AND NOT IN THE WHOLE IMAGE!!!!!!!!!!!!!!!!! CHANGE IT'''
    
    print '\n'
    print 'Sort of spheres'
    
    # Get the (x,y) centres of spheres
    
    centroids_sep = [] # to stock the centres one by one
    for slice in range(N):
        
        for i in range(len(centroids_sphere[slice])):
            #print centroids_sphere[slice][i],
    ...
    print 'All centres sorted:', centroids_sep

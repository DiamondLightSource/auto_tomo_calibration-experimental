...
def detect_spheres(image):
    ...
    for n in range(nb_spheres):
        ...
        for i in range(0, len(areas[n])):
            ...
            # largest_areas[n]/2.0 - center_y gets the centre position for all of the
            # segmented areas centre+border gives the absolute centre in the image
            circle_slice[circle_full[0] + largest_areas[n] / 2 - center_x, \
                          circle_full[1] + largest_areas[n] / 2 - center_y, \
                          slices[n][i] - min(slices[n])] = 255
            
        # Sort the slices according to radius
        ...
        while not successful:    
            ...
            for i in range(0, len(R) / 2 - 1):
                if R[i] > R[i + 1]:
                    cp = np.copy(circle_slice[:,:,i + 1])
                    circle_slice[:,:,i + 1] = circle_slice[:,:,i]
                    circle_slice[:,:,i] = cp
...
centre = [(30,30,30), (70,30,30)]#, (151,151,151)]
radius = [15,20]#,30,40]
value = 20

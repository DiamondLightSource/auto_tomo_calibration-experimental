...
def select_area_sphere(np_image): # to select the cubes containing the spheres
    ...
    """
	delete wrong aras for each slice
	"""
	...
	just centroid values. Modifies the data depending on seperation
	"""
    centroids_sep = []
    for slice in range(N):
    ...
    """
	centroids_sphere stores the centres for each slice. It is an array for each slice
	centroids stores the centres having a correct separation
	
	this loop does... for each sphere it loops through the slices
	takes the centroids and checks if they are within a proper region
	
	this separates the spehres into different arrays, since before they were all together
	"""
